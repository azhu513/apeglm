...
rbind(summary(abs(fit$map[,5] - fitR$map[,5])),
      summary(abs(fit$sd[,5] - fitR$sd[,5])))
```

```{r eval=FALSE, echo=FALSE}
#The version `nbinomCR` is ~8 times faster than the default `general`. 
system.time({
...
rbind(summary(abs(fit$map[,5] - fitCR$map[,5])),
      summary(abs(fit$sd[,5] - fitCR$sd[,5])))
```

```{r eval=FALSE, echo=FALSE}
#The version `nbinomC` returns only the MAP coefficients and
#can be more than 60 times faster than the default `general`. The MAP
#coefficients are the same as returned by `nbinomCR` above, we just
#skip the calculation of posterior SD.
system.time({
