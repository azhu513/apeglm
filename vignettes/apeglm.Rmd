...
# we are working on faster implementations of apeglm
# this version is 3-4x faster but has issues with MAP and Hessian right now
system.time({
  fit.fast <- apeglm(Y=assay(airway)[5:10,], x=x,
                     log.lik=NULL,
                     param=param[5:10],
                     coef=ncol(x),
                     threshold=log(2) * 1,
...
logit(.75)
table(abs(logit(prob[sofsval < .01])) > .5)
```
