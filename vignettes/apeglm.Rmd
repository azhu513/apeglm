...
or by number (following the order in `resultsNames(dds)`).
...
binomial likelihoods. The version `method="nbinomR"` is ~5 times
...
#plot(fit$map[,5], fitR$map[,5])
summary(abs(fit$map[,5] - fitR$map[,5]))
sum(is.na(fitR$sd[,1]))
```

```{r, eval=FALSE, echo=FALSE}
# The version `method="nbinomCR"` is ~8 times faster than the default `method="general"`. 
system.time({
...
plot(fit$map[,5], fitCR$map[,5])
summary(abs(fit$map[,5] - fitCR$map[,5]))
sum(is.na(fitCR$sd[,1]))
```

```{r, eval=FALSE, echo=FALSE}
# The version `method="nbinomC"` returns only the MAP coefficients and
# is ~50 times faster than the default `method="general"`. 
system.time({
...
summary(abs(fit$map[,5] - fitC$map[,5]))
```
...
than zero, and asking for the probability of a "false-sign-or-small" event:
...
per-gene probabilities to produce a *fsos*-value.
...
```{r maplotthresh}
fsos.val <- svalue(fit$thresh)
cols <- ifelse(fsos.val < .01, "red", "black")
...
probability of false-sign-or-small events (*fsos*-value < .01), where
...
logit(.75)
table(abs(logit(prob[fofs.val < .01])) > .5)
```
