...
We are working on faster implementations of apeglm. This version
`method="nbinomR"` is 5-6x faster than the default `method="general"`.

```{r}
system.time({
  fit.fast <- apeglm(Y=assay(airway), x=x,
                     log.lik=NULL,
                     param=param,
                     coef=ncol(x),
                     threshold=log(2) * 1,
...
summary(abs(fit$map[,5] - fit.fast$map[,5]))
sum(is.na(fit$sd[,1]))
sum(is.na(fit.fast$sd[,1]))
```
